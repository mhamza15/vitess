...
func registerCommands(root *cobra.Command) {
	...
	create.Flags().BoolVar(&createOptions.AllViews, "all-views", false, "Copy all views from the source.")
	create.Flags().StringSliceVar(&createOptions.IncludeViews, "views", nil, "Source views to copy.")
	create.Flags().StringSliceVar(&createOptions.ExcludeViews, "exclude-views", nil, "Source views to exclude from copying.")
	create.Flags().BoolVar(&createOptions.SkipViewValidation, "skip-view-validation", false, "Skip validating that views reference tables being moved.")
