...
func (hook *Hook) findHook(ctx context.Context) (*exec.Cmd, int, error) {
	...
	log.InfoS().Msgf("hook: executing hook: %v %v", vthook, strings.Join(hook.Parameters, " "))
...
func (hook *Hook) ExecuteContext(ctx context.Context) (result *HookResult) {
	...
	defer func() {
		log.InfoS().Msgf("hook: result is %v", result.String())
	}()
...
func (hook *Hook) ExecuteOptional() error {
	...
	case HOOK_DOES_NOT_EXIST:
		log.InfoS().Msgf("%v hook doesn't exist", hook.Name)
	case HOOK_VTROOT_ERROR:
		log.InfoS().Msgf("VTROOT not set, so %v hook doesn't exist", hook.Name)
	case HOOK_SUCCESS:
		// nothing to do here
	default:
