...
func testCommitError(t *testing.T, conn *mysql.Conn, clusterInstance *vitesst.Cluster, tablets []*vitesst.Tablet) {
...
func testExecuteError(t *testing.T, conn *mysql.Conn, clusterInstance *vitesst.Cluster, tablets []*vitesst.Tablet) {
	tabletStopped := make(chan bool)
	...
	go func() {
		...
		idx += 5
		<-tabletStopped
		_, err := conn.ExecuteFetch(GetInsertMultipleValuesQuery(idx, idx+1, idx+2, idx+3), 0, false)
		if !assert.ErrorContains(t, err, "VT15001") {
			return
		}
...
func testExecuteErrorWhileTabletIsNotServing(t *testing.T, conn *mysql.Conn, clusterInstance *vitesst.Cluster, tablets []*vitesst.Tablet) {
...
func reparent(t *testing.T, clusterInstance *vitesst.Cluster, tablets []*vitesst.Tablet, tabletStopped, actionDone chan bool) {
	...
	output, err := Prs(t, clusterInstance, tablets[prsTo])
...
func makeTabletNotServing(t *testing.T, clusterInstance *vitesst.Cluster, tablets []*vitesst.Tablet, stateChanged, actionDone chan bool) {
	// Reparent to the other replica
	ShardName = "40-80"
	defer func() {
		ShardName = "0"
...
func TestErrorsInTransaction(t *testing.T) {
	clusterInstance := SetupShardedReparentCluster(t, policy.DurabilitySemiSync, []string{
	...
	tablets := keyspace.Shards()[1].Tablets()
	...
	ConfirmReplication(t, tablets[primary], []*vitesst.Tablet{tablets[1], tablets[2]})
