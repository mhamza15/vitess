...
func TestSimpleInsertSelect(t *testing.T) {
	...
	vitesst.AssertMatches(t, mcmp.VtConn, `select num from num_vdx_tbl order by num`, `[[INT64(2)] [INT64(4)] [INT64(40)] [INT64(42)] [INT64(80)] [INT64(84)]]`)
}
...
func TestFailureInsertSelect(t *testing.T) {
	...
	for _, mode := range []string{"oltp", "olap"} {
		mcmp.Run(mode, func(mcmp *vitesst.MySQLCompare) {
			...
			vitesst.AssertContainsError(t, mcmp.VtConn, "insert into s_tbl(id, num) select id*20, num from s_tbl where id = 1", `(errno 1062) (sqlstate 23000)`)
...
func TestAutoIncInsertSelect(t *testing.T) {
	...
	vitesst.Exec(t, mcmp.VtConn, "insert into user_tbl(region_id, name) values (1,'A'),(3,'C')")
	...
	for _, tcase := range tcases {
		mcmp.Run(tcase.query, func(mcmp *vitesst.MySQLCompare) {
			qr := vitesst.Exec(t, mcmp.VtConn, tcase.query)
...
func TestUnownedVindexInsertSelect(t *testing.T) {
	...
	vitesst.Exec(t, mcmp.VtConn, "insert into order_tbl(oid, region_id) values (100,1),(300,2),(600,3),(700,4)")

	qr := vitesst.Exec(t, mcmp.VtConn, "insert into oevent_tbl(oid, ename) select oid, 'dispatched' from order_tbl")
	assert.EqualValues(t, 4, qr.RowsAffected)

	vitesst.Exec(t, mcmp.VtConn, "use `sks/-80`")
	qr = vitesst.Exec(t, mcmp.VtConn, `select count(*) from order_tbl o join oevent_tbl oe on o.oid = oe.oid`)
	...
	vitesst.Exec(t, mcmp.VtConn, "use `sks`")

	// inserting non-existing record in oevent_tbl.
	vitesst.AssertContainsError(t, mcmp.VtConn, "insert into oevent_tbl(oid, ename) select 1000, 'dispatched'", `could not map [INT64(1000)] to a keyspace id`)
	...
	qr = vitesst.Exec(t, mcmp.VtConn, "insert into oextra_tbl(id, oid) select region_id, oid from order_tbl")
	assert.EqualValues(t, 4, qr.RowsAffected)

	// mismatch value, verify step will fail. oid 100 is mapped to region_id 1, in this test trying to insert 100 with 2.
	vitesst.AssertContainsError(t, mcmp.VtConn, "insert into oextra_tbl(id, oid) select 2, 100", `values [[INT64(100)]] for column [oid] does not map to keyspace ids`)

	// null to oid is allowed so verify ksids will ignore it.
	vitesst.Exec(t, mcmp.VtConn, "insert into oextra_tbl(id, oid) select 5, null")
}
...
func TestUnownedVindexInsertSelectOlapMode(t *testing.T) {
	...
	vitesst.Exec(t, mcmp.VtConn, "use `sks/-80`")
	vitesst.AssertMatches(t, mcmp.VtConn, `select count(*) from order_tbl o join oevent_tbl oe on o.oid = oe.oid`, `[[INT64(3)]]`)
	vitesst.Exec(t, mcmp.VtConn, "use `sks/80-`")
	vitesst.AssertMatches(t, mcmp.VtConn, `select count(*) from order_tbl o join oevent_tbl oe on o.oid = oe.oid`, `[[INT64(1)]]`)
...
func TestLookupCasesIncInsertSelect(t *testing.T) {
	...
	vitesst.AssertMatches(t, mcmp.VtConn, `select unq_col from unq_idx order by unq_col`,
		`[[INT64(2)] [INT64(3)]]`)
	vitesst.AssertMatches(t, mcmp.VtConn, `select nonunq_col, id from nonunq_idx order by nonunq_col, id`,
...
func TestIgnoreInsertSelect(t *testing.T) {
	...
	qr = vitesst.Exec(t, mcmp.VtConn, "insert into order_tbl(region_id, oid, cust_no) select region_id, oid, cust_no from order_tbl where oid = 4 on duplicate key update cust_no = cust_no + 1")
	assert.EqualValues(t, 2, qr.RowsAffected)
	vitesst.AssertMatches(t, mcmp.VtConn, "select count(*) from order_tbl", `[[INT64(4)]]`)

	vitesst.AssertMatches(t, mcmp.VtConn, "select oid, cust_no from order_tbl order by oid", `[[INT64(1) INT64(100)] [INT64(2) INT64(200)] [INT64(3) INT64(300)] [INT64(4) INT64(401)]]`)

	// inserting on dup trying to update vindex throws error.
	vitesst.AssertContainsError(t, mcmp.VtConn, "insert into order_tbl(region_id, oid, cust_no) select 1, 10, 1000 on duplicate key update region_id = region_id + 1", `unsupported: DML cannot update vindex column`)
	vitesst.AssertContainsError(t, mcmp.VtConn, "insert into order_tbl(region_id, oid, cust_no) select 1, 10, 1000 on duplicate key update oid = oid + 100", `unsupported: DML cannot update vindex column`)
}
...
func TestMixedCases(t *testing.T) {
	...
	for _, tc := range tcases {
		mcmp.Run(tc.insQuery, func(mcmp *vitesst.MySQLCompare) {
			vitesst.Exec(t, mcmp.VtConn, tc.insQuery)
			vitesst.AssertMatches(t, mcmp.VtConn, tc.selQuery, tc.exp)
		})
	}

	// final check count on the lookup vindex table.
	vitesst.AssertMatches(t, mcmp.VtConn, "select count(*) from lkp_mixed_idx", "[[INT64(12)]]")
}
...
func TestInsertJson(t *testing.T) {
	...
	vitesst.Exec(t, mcmp.VtConn, `insert into uks.j_utbl(id, jdoc) select * from sks.j_tbl`)
	vitesst.AssertMatches(t, mcmp.VtConn, `select * from uks.j_utbl order by id`,
...
func TestInsertIgnoreNullAndInsertNull(t *testing.T) {
	...
	mcmp.Exec("select id, num, col from s_tbl")
	vitesst.AssertMatches(t, mcmp.VtConn, "select num, hex(keyspace_id) from num_vdx_tbl order by num",
	...
	vitesst.AssertMatches(t, mcmp.VtConn, "select col, id, hex(keyspace_id) from col_vdx_tbl order by col, id",
	...
	vitesst.AssertMatches(t, mcmp.VtConn, "select name, id, hex(keyspace_id) from name_vdx_tbl order by name, id",
		`[[VARCHAR("bar") INT64(2) VARCHAR("06E7EA22CE92708F")] [VARCHAR("foo") INT64(1) VARCHAR("166B40B44ABA4BD6")]]`)

	_, err := vitesst.ExecAllowError(t, mcmp.VtConn, "insert ignore into name_tbl(id, name) values (22, NULL)") // this should fail, since we have a vindex that does not ignore nulls
