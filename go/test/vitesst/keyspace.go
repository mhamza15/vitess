...
type keyspaceConfig struct {
	...
	// schema is applied after tablets and vtgate are running.
	schema string

	// vschema is applied before tablets start so vtgate can route queries.
	vschema string

	// shardCount controls how many shard ranges are generated.
	shardCount int

	// replicaCount controls how many tablets are started per shard.
	replicaCount int
...
type keyspaceBuilder struct {
	// config accumulates the keyspace settings until apply copies them into
	// clusterOptions.
